...
### 5.2 多端自适应布局 (Responsive App Layout)

```
[ Desktop & iPad 竖屏 (>= 768px, md:) ]
┌─────────────────────────────────────────────┐
...
│  (h-[45vh], 顶部大圆角 rounded-t-3xl)        │
└─────────────────────────────────────────────┘
```

为了确保这种响应式能在手机端正确推压内容，最外层 `stk-workspace` 使用了 `flex-col-reverse md:flex-row`。
...
### 5.4 控制面板 / 侧边栏 (Sidebar / Aside)

控制面板在桌面端位于左侧，在手机端化身为位于底部的固定高度抽屉。为防止手机端滚轮被外层拦截，必须添加 `data-lenis-prevent="true"`。

| 属性 | 值 |
|------|----|
| 尺寸 | `w-full md:w-72`，高度 `h-[45vh] md:h-auto` |
| 背景 | `bg-[#0d0d12]` |
| 边线 | 桌面端 `border-r border-white/5`，手机端改为 `border-t` |
| 圆角 | 手机端顶部圆角 `rounded-t-3xl md:rounded-none` |
| 内边距 | `p-4 md:p-6` |
| 溢出 | `overflow-y-auto` + `custom-scrollbar` |
| 布局 | `flex-col shrink-0 relative z-10` |
...
### 5.9 主预览区交互与浮动控件 (Canvas Interactions & Controls)
...
1. **移动端“假全屏”逻辑 (Mobile Fullscreen)**
   在手机端（`< 768px`），点击“全屏切换”按钮**不应该**触发浏览器原生的全屏 API（容易因刘海屏或地址栏出现兼容问题）。
   正确的处理方式是切换 `mobileFullscreen` 状态：当开启时，将底部的控制面板隐藏 (`hidden`)，让画布独占整个视口空间。

2. **多点触控与双指捏合 (Multi-touch Pinch to Zoom)**
   对于支持无限画布的工具，必须利用 React 的 `onPointerDown`/`Move`/`Up` 事件维护一个活动指针映射（`pointersRef = new Map()`）。
   当检测到两个活动指针时，停止单指平移（Pan），计算两指间距变化（`Math.hypot`）与中心点，执行以双指中心为锚点的极度顺滑的缩放逻辑（Zoom）。这对于移动端画布体验至关重要。
