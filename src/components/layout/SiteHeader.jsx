...
function Logo() {
  ...
  return (
    <Link to="/" className="block" ref={logoRef}>
      <svg
        width="120"
        height="122"
        viewBox="0 0 400 406"
...
export default function SiteHeader() {
  ...
  return (
    <header className="dcde-grid-12 items-end">
      {/* Logo */}
      <div className="col-span-3 row-span-2" style={{ gridColumn: '1 / 4', gridRow: '1 / 3' }}>
        <Logo />
      </div>

      {/* Mission Statement */}
      <div className="col-span-9" style={{ gridColumn: '4 / 13', gridRow: '1' }}>
        <div className="flex flex-col justify-between h-full pb-16">
          <h1 className="dcde-text-lg mb-5">
            我们欣赏每一个独立灵魂，并为独立团队提供设计支持。<br />
            通过 DCDE 的视角，重塑原创的视觉资产。
          </h1>
          <p className="dcde-text-sm text-grey-text">
      ...
      {navItems.map((item) => (
        <Link
          key={item.path}
          to={item.path}
          className={`dcde-nav-item dcde-text-sm self-end ${isActive(item) ? 'active dcde-text-accent' : ''}`}
          style={{
            gridColumn: item.path === '/' ? '4 / 6' : item.path === '/about' ? '6 / 12' : '11 / 13',
            gridRow: '2',
          }}
        >
          {item.label}
